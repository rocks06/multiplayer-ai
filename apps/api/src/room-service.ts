...
export class RoomService {
  ...
  async snapshot(companyId:string,roomId:string,actorId:string) {
    ...
    try {
      ...
      const members=await c.query(`SELECT rm.principal_id,p.display_name,p.kind,rm.role,rm.responsibilities,s.status agent_connection,s.runtime_status agent_runtime_status,s.last_seen_at agent_last_seen_at,CASE WHEN p.kind<>'agent' THEN NULL WHEN s.status IS NULL THEN 'never' WHEN s.status<>'connected' THEN s.status WHEN s.last_seen_at < now()-interval '90 seconds' THEN 'stale' ELSE 'connected' END agent_presence FROM room_members rm JOIN principals p ON p.id=rm.principal_id LEFT JOIN LATERAL (SELECT es.status,es.runtime_status,es.last_seen_at FROM external_agent_sessions es WHERE es.company_id=rm.company_id AND es.room_id=rm.room_id AND es.agent_principal_id=rm.principal_id ORDER BY es.last_seen_at DESC LIMIT 1) s ON true WHERE rm.room_id=$1 AND rm.company_id=$2 AND rm.status='active' ORDER BY rm.joined_at`,[roomId,companyId]);
      ...
      const messages=await c.query(`SELECT m.id,m.sender_principal_id,m.addressed_principal_id,m.body_text,m.task_id,m.in_reply_to_message_id,m.created_at,p.display_name sender_name,p.kind sender_kind FROM messages m JOIN principals p ON p.id=m.sender_principal_id WHERE m.room_id=$1 AND m.company_id=$2 ORDER BY m.created_at DESC LIMIT 50`,[roomId,companyId]);
