...
interface CommandContext { companyId: string; roomId: string; actorId: string; idempotencyKey: string; commandType: string; input: unknown; permission: Permission; }

export class RoomService {
  constructor(private readonly pool: DbPool) {}

  private async actor(client: DbClient, companyId: string, actorId: string): Promise<Actor> {
    const result = await client.query<Actor>(`SELECT id, company_id, kind, display_name FROM principals WHERE id=$1 AND company_id=$2 AND status='active'`, [actorId, companyId]);
  ...
  private async membership(client: DbClient, companyId: string, roomId: string, actorId: string): Promise<Membership> {
    const result = await client.query<Membership>(`SELECT role, responsibilities FROM room_members WHERE company_id=$1 AND room_id=$2 AND principal_id=$3 AND status='active'`, [companyId, roomId, actorId]);
  ...
  private async command<T>(ctx: CommandContext, run: (client:DbClient, actor:Actor, commandId:string)=>Promise<{response:T; event:{type:string; entityType:string; entityId:string; entityVersion?:number; payload:unknown}}>): Promise<T & {event_id:string; room_seq:number; command_id:string}> {
    ...
    try {
      ...
      const requestDigest = digest(ctx.input);
      const commandId = uuidv7();
      const inserted = await client.query(`INSERT INTO command_receipts (command_id,company_id,principal_id,idempotency_key,command_type,request_digest,response_status,response_body) VALUES ($1,$2,$3,$4,$5,$6,0,'{}'::jsonb) ON CONFLICT (company_id,principal_id,idempotency_key) DO NOTHING RETURNING command_id`, [commandId,ctx.companyId,ctx.actorId,ctx.idempotencyKey,ctx.commandType,requestDigest]);
      if (!inserted.rowCount) {
        const prior = await client.query<{command_id:string;request_digest:string;response_body:T & {event_id:string;room_seq:number;command_id:string}}>(`SELECT command_id,request_digest,response_body FROM command_receipts WHERE company_id=$1 AND principal_id=$2 AND idempotency_key=$3`, [ctx.companyId,ctx.actorId,ctx.idempotencyKey]);
  ...
  async updateTaskStatus(input:{companyId:string;roomId:string;actorId:string;taskId:string;status:TaskStatus;expectedVersion:number;idempotencyKey:string}) { const client=await this.pool.connect(); let permission:Permission='task.update.own'; try { const m=await this.membership(client,input.companyId,input.roomId,input.actorId); if(roleHasPermission(m.role,'task.update.any')) permission='task.update.any'; } finally{client.release();} return this.command({...input,commandType:'task.status.update',input:{taskId:input.taskId,status:input.status,expectedVersion:input.expectedVersion},permission}, async(c)=>{ const current=await c.query<{status:TaskStatus;version:number;assignee_principal_id:string|null}>(`SELECT status,version,assignee_principal_id FROM tasks WHERE id=$1 AND company_id=$2 AND room_id=$3`,[input.taskId,input.companyId,input.roomId]); if(!current.rowCount)throw new DomainError('task_not_found','Task not found',404); const task=current.rows[0]!; if(permission==='task.update.own' && task.assignee_principal_id!==input.actorId)throw new DomainError('permission_denied','Only the assignee may update this task',403); if(task.version!==input.expectedVersion)throw new DomainError('version_conflict','Task changed since it was read',409,{expected_version:input.expectedVersion,current_version:task.version}); if(!canTransitionTask(task.status,input.status))throw new DomainError('invalid_task_transition',`Cannot transition ${task.status} to ${input.status}`,422); const nextVersion=task.version+1; await c.query(`UPDATE tasks SET status=$1,version=$2,updated_at=now(),completed_at=CASE WHEN $1='completed' THEN now() ELSE completed_at END WHERE id=$3`,[input.status,nextVersion,input.taskId]); const response={id:input.taskId,status:input.status,version:nextVersion}; return {response,event:{type:`task.${input.status}`,entityType:'task',entityId:input.taskId,entityVersion:nextVersion,payload:response}}; }); }
  ...
  async snapshot(companyId:string,roomId:string,actorId:string) {
    ...
    try {
      ...
      return {room:room.rows[0],members:members.rows,tasks:tasks.rows,messages:messages.rows.reverse(),snapshot_seq:Number(room.rows[0].last_event_seq),briefing:{briefing_seq:Number(room.rows[0].last_event_seq),project_objective:room.rows[0].objective,participants:members.rows,joining_principal:{principal_id:actorId,role:member.role,responsibilities:member.responsibilities},active_tasks:active,relevant_completed_work:completed,unresolved_decisions:[],blockers:active.filter((t:any)=>t.status==='blocked'),relevant_artifacts:[],important_recent_activity:events.rows.reverse()}};
    } finally { c.release(); }
  }
