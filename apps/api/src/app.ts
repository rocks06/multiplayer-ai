...
export function buildApp(pool:DbPool=createPool(), realtimeOptions:RealtimeOptions={}) {
  ...
  app.post('/v1/companies/:companyId/rooms/:roomId/decisions/:decisionId/approve',async req=>{const p=body(z.object({companyId:z.string().uuid(),roomId:z.string().uuid(),decisionId:z.string().uuid()}),req.params);const x=body(resolutionBody,req.body);return agentRuntime.resolveDecision({companyId:p.companyId,roomId:p.roomId,actorId:principal(req),decisionId:p.decisionId,resolution:'approved',proposedActionDigest:x.proposed_action_digest,expectedVersion:x.expected_version,note:x.note,idempotencyKey:idem(req)})});
  app.post('/v1/companies/:companyId/rooms/:roomId/decisions/:decisionId/reject',async req=>{const p=body(z.object({companyId:z.string().uuid(),roomId:z.string().uuid(),decisionId:z.string().uuid()}),req.params);const x=body(resolutionBody,req.body);return agentRuntime.resolveDecision({companyId:p.companyId,roomId:p.roomId,actorId:principal(req),decisionId:p.decisionId,resolution:'rejected',proposedActionDigest:x.proposed_action_digest,expectedVersion:x.expected_version,note:x.note,idempotencyKey:idem(req)})});
  app.post('/v1/companies/:companyId/rooms/:roomId/decisions/:decisionId/cancel',async req=>{const p=body(z.object({companyId:z.string().uuid(),roomId:z.string().uuid(),decisionId:z.string().uuid()}),req.params);const x=body(z.object({expected_version:z.number().int().positive()}),req.body);return agentRuntime.cancelDecision({companyId:p.companyId,roomId:p.roomId,actorId:principal(req),decisionId:p.decisionId,expectedVersion:x.expected_version,idempotencyKey:idem(req)})});
