...
const agentA:Member={principal_id:AGENT_A,display_name:'Agent A',kind:'agent',role:'worker_agent',responsibilities:''};
const agentB:Member={principal_id:AGENT_B,display_name:'Agent B',kind:'agent',role:'worker_agent',responsibilities:''};

const decision=(o:Partial<Decision>&{id:string}):Decision=>({
  run_id:null,requested_by_principal_id:AGENT_A,title:'Publish the 429 contract now?',
...
const cases:Array<{name:string;decisions:Decision[];requester:(d:Decision)=>Member}>=[
  {name:'One pending decision',decisions:[decision({id:'f1'})],requester:()=>agentA},
  {name:'Several, from different agents',decisions:[
    decision({id:'f2',requested_by_principal_id:AGENT_A,title:'Publish the 429 contract now?',requested_at:minutesAgo(9)}),
    decision({id:'f3',requested_by_principal_id:AGENT_B,title:'Drop the Retry-After header from the published contract?',requested_at:minutesAgo(4)}),
    decision({id:'f4',requested_by_principal_id:AGENT_B,title:'Ship the docs PR without the platform team review?',requested_at:minutesAgo(1)}),
  ],requester:d=>d.requested_by_principal_id===AGENT_B?agentB:agentA},
  {name:'Long question and action',decisions:[decision({id:'f5',title:'Publish a computed Retry-After the proxy can invalidate?',question:longQuestion,proposed_action:longAction,rationale:'The launch date depends on publishing something, and an approximate header is more useful to callers than none — but only if the caveat is documented alongside it.'})],requester:()=>agentA},
];
