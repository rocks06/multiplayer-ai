...
describe('Vertical Slice 4 human decision and approval workflow',()=>{
 ...
 it('exposes scoped list/read and returns HTTP 403 for unauthorized approval',async()=>{
  const s=await fixture();const run=await queue(s,s.agentA,s.taskA,[request()],'http-decision');await worker('request').runOnce();const d=await decisionFor(run.id);const apiPool=new pg.Pool({connectionString});const app=buildApp(apiPool,{pollIntervalMs:50});await app.ready();
