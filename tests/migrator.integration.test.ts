...
describe("Database migrator", () => {
  ...
  /** A repository whose migrations stop at `through`, so an older database can be simulated. */
  async function repoUpTo(through: string) {
  ...
  it("applies only the migrations an existing database has not seen", async () => {
    const pool = connect();
    try {
      // A database that was initialized when the newest migration was 0006.
      const older = await repoUpTo("0006_human_authentication.sql");
      await migrate(pool, older);
      await pool.query(`ALTER TABLE tasks DROP COLUMN IF EXISTS dependency_override_at, DROP COLUMN IF EXISTS dependency_override_by_principal_id`);
      ...
      expect(upgraded.applied).toEqual(["0007_task_dependencies.sql"]);

      const columns = await pool.query(`SELECT 1 FROM information_schema.columns WHERE table_name='tasks' AND column_name='dependency_override_at'`);
      expect(columns.rowCount).toBe(1);
      const table = await pool.query(`SELECT 1 FROM pg_tables WHERE tablename='task_dependencies'`);
  ...
  it("refuses a migration that changed after it was applied", async () => {
    ...
    try {
      await migrate(pool);
      const tampered = await repoUpTo("9999");
