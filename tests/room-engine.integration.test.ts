...
async function setup(){
 const company=(await seedCompany(pool, ({name:'Acme'}).name));
...
describe('Phase 1A vertical slice',()=>{
 ...
 it('enforces room isolation for snapshots and event replay',async()=>{const a=await setup(); const outsiderCompany=(await seedCompany(pool, ({name:'Other'}).name)); const outsider=(await seedHuman(pool, outsiderCompany.id, ({email:`out-${crypto.randomUUID()}@example.com`,display_name:'Outsider'}).email, ({email:`out-${crypto.randomUUID()}@example.com`,display_name:'Outsider'}).display_name)); const snap=await app.inject({method:'GET',url:`/v1/companies/${a.company.id}/rooms/${a.room.id}/snapshot`,headers:{'x-principal-id':outsider.principal_id}}); const events=await app.inject({method:'GET',url:`/v1/companies/${a.company.id}/rooms/${a.room.id}/events`,headers:{'x-principal-id':outsider.principal_id}}); expect(snap.statusCode).toBe(403); expect(events.statusCode).toBe(403);});
