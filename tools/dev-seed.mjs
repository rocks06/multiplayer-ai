...
try {
  ...
  for (const name of ['Agent A', 'Agent B']) {
    ...
    await rooms.addMember({ companyId: company, roomId: room.id, actorId: me, principalId: agent.principal_id, role: 'worker_agent', responsibilities: name === 'Agent A' ? 'Research and technical investigation' : 'Integration and implementation', idempotencyKey: key() });
    agents[name] = agent;
    ...
    await pool.query(
      ...
      [randomUUID(), company, agent.principal_id, room.id, [...randomUUID().replace(/-/g, ''), ...randomUUID().replace(/-/g, '')].join('').slice(0, 64), name === 'Agent A' ? 'working' : 'idle']);
  }

  const investigate = await rooms.createTask({ companyId: company, roomId: room.id, actorId: me, title: 'Investigate rate-limit behaviour under burst load', description: 'Establish what the current gateway does when a client exceeds its quota in a short window.', assigneePrincipalId: agents.Agent A.principal_id, idempotencyKey: key() });
  const design = await rooms.createTask({ companyId: company, roomId: room.id, actorId: me, title: 'Design the published quota contract', description: 'Turn the findings into the contract we publish to developers.', assigneePrincipalId: agents.AGENT_B.principal_id, idempotencyKey: key() });
  // AGENT_B genuinely cannot start until Agent A reports: a modelled dependency, not an inference.
  ...
  await rooms.updateTaskStatus({ companyId: company, roomId: room.id, actorId: agents.Agent A.principal_id, taskId: investigate.id, status: 'in_progress', expectedVersion: 1, idempotencyKey: key() });

  const opening = await rooms.sendMessage({ companyId: company, roomId: room.id, actorId: agents.Agent A.principal_id, addressedPrincipalId: agents.AGENT_B.principal_id, body: 'Starting on the rate-limit investigation. Early signal: the gateway drops the connection rather than returning 429, so the published contract cannot promise a retry header yet.', idempotencyKey: key() });
  const jjHold = await rooms.sendMessage({ companyId: company, roomId: room.id, actorId: agents.AGENT_B.principal_id, addressedPrincipalId: agents.Agent A.principal_id, inReplyToMessageId: opening.id, body: 'Understood. I will hold the contract draft until you confirm whether that is the gateway or the upstream proxy — it changes what we can commit to.', idempotencyKey: key() });
  ...
  await rooms.sendMessage({ companyId: company, roomId: room.id, actorId: agents.AGENT_B.principal_id, addressedPrincipalId: me, inReplyToMessageId: interruption.id, body: 'Per region, from the configuration Agent A pulled. A single client can be throttled by traffic it did not generate, which we should say plainly in the docs rather than bury.', idempotencyKey: key() });
  await rooms.sendMessage({ companyId: company, roomId: room.id, actorId: agents.Agent A.principal_id, body: 'Noting for the room: the proxy configuration is owned by the platform team, so any fix there needs their sign-off and will not land inside this launch window.', idempotencyKey: key() });
  await rooms.sendMessage({ companyId: company, roomId: room.id, actorId: agents.AGENT_B.principal_id, addressedPrincipalId: agents.Agent A.principal_id, inReplyToMessageId: opening.id, body: 'Coming back to your first point about the retry header — given the per-region limit, a Retry-After we compute at the gateway would be wrong often enough to be worse than omitting it.', idempotencyKey: key() });

  const decision = await runtime.requestExternalDecision({
    companyId: company, roomId: room.id, actorId: agents.Agent A.principal_id,
  ...
  await rooms.updateTaskStatus({ companyId: company, roomId: room.id, actorId: agents.Agent A.principal_id, taskId: investigate.id, status: 'awaiting_decision', expectedVersion: 2, idempotencyKey: key() });
