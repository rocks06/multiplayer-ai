...
const assignedTaskEvent=(id,assignee,seq=11)=>({id,room_seq:seq,event_type:'task.created',actor_principal_id:human,actor_kind:'human',entity_type:'task',entity_id:taskId,payload:{id:taskId,title:'Physical retest',status:'open',version:1,assignee_principal_id:assignee}});
results.push(await scenario('task-assignment-offline-replay',{event:assignedTaskEvent('event-task-created-replay-11',principal),send:'replay'}));
results.push(await scenario('task-assignment-duplicate-replay',{event:assignedTaskEvent('event-task-created-dup-11',principal),send:'live',duplicate:true}));
results.push(await scenario('task-assignment-hermes-failure-retry',{event:assignedTaskEvent('event-task-created-fail-11',principal),send:'live',exitPlan:[1,0],inspectFailure:true}));
results.push(await scenario('task-assignment-other-agent',{event:assignedTaskEvent('event-task-created-peer-11',peer),send:'live',expectWake:false}));
